  ...
  return (
    <div>
      <h1 className="text-5xl font-bold text-white max-w-xl">
        Job <span className="text-[#7A38F0]">Opportunity</span>
      </h1>
      ...
      <div className="mt-10 space-y-4">
        {opportunities.map((item, index) => (
          ...
          >
            ...
            >
              <h1 className="text-3xl font-semibold text-white tracking-widest">
                {item.title}
              </h1>
              <FaChevronCircleDown
                className={`text-4xl text-[#7A38F0] transition-transform duration-300 ${openIndex === index ? "rotate-180" : ""}`}
              />
            </div>

            {openIndex === index && (
              <div className="p-6 pt-2 text-white/90">
                <p className="text-lg mb-4">{item.description}</p>

                <ul className="space-y-2 mb-6">
                  {item.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <span className="text-[#7A38F0] mt-1.5">•</span>
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
